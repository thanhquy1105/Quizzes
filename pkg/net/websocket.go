...
func (w *WSConn) WriteServerBinary(data []byte) error {
	w.mu.Lock()
	defer w.mu.Unlock()
	err := wsutil.WriteServerBinary(w.outboundBuffer, data)
	if err != nil {
		return err
	}
...
func (w *WSConn) WriteServerText(data []byte) error {
	w.mu.Lock()
	defer w.mu.Unlock()
	err := wsutil.WriteServerText(w.outboundBuffer, data)
	if err != nil {
		return err
	}
	return w.addWriteIfNotExist()
...
func (w *WSConn) upgrade() error {
	...
	if strings.TrimSpace(realIp) != "" {
		...
		if strings.TrimSpace(realPortStr) != "" {
			realPort = util.ParseInt(realPortStr)
		} else {
			if w.remoteAddr != nil {
